---
to: admin/src/services/<%= h.changeCase.param(name) %>.ts
---
import { API_URL } from 'config/constants';
import { encodeFilter } from 'config/encoding';
import { usePaginatedQuery, useMutation, useQueryCache } from 'react-query';

const resourceUrl = 'api/v1/admin/<%= h.changeCase.param(name) %>';

export function use<%= h.changeCase.pascal(name) %>List(
  _page = 0,
  _limit = 20,
  _search = '',
  _filters = [],
  ...
  return usePaginatedQuery(
    ['<%= h.changeCase.camel(name) %>List', _page, _limit, filters, search],
    (key, page, limit) =>
      fetch(
...
export function use<%= h.changeCase.pascal(name) %>Create() {
...
export function use<%= h.changeCase.pascal(name) %>Update() {
  const cache = useQueryCache();
  const createRequest = body => {
    const updateBody = {
      ...body,

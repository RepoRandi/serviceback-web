...
const resourceUrl = 'api/v1/admin/service-category';
...
export function useServiceCategoryList(
  ...
  return usePaginatedQuery(
    ['serviceCategoryList', _page, _limit, filters, search],
    (key, page, limit) =>
      fetch(
...
export function useServiceCategoryCreate() {
...
export function useServiceCategoryUpdate() {
  const cache = useQueryCache();
  const createRequest = body => {
    const updateBody = {
      ...body,

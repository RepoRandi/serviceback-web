...
import { SERVICE_STATUS_OPTIONS } from 'config/constants';
...
export default function Service() {
  ...
  const createItem = useCallback(
    () =>
      dispatch({ type: 'OPEN_DRAWER', drawerComponent: 'SERVICE_CREATE_FORM' }),
    [dispatch],
  );
  const editItem = useCallback(
    item =>
      dispatch({
        type: 'OPEN_DRAWER',
        drawerComponent: 'SERVICE_UPDATE_FORM',
        data: item,
      }),
    [dispatch],
  );
  ...
  useEffect(() => {
    if (debouncedSearchTerm) {
      cache.invalidateQueries('serviceList');
    }
  ...
  const { error, data } = useServiceList(
    page,
    limit,
    debouncedSearchTerm,
    filter,
